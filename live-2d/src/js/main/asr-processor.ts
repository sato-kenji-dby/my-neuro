...
class ASRProcessor {
  ...
  constructor(vadUrl: string, asrUrl: string) {
    ...
    this.BUFFER_SIZE = Math.floor(
      this.SAMPLE_RATE * (this.BUFFER_DURATION / 1000)
    );
    ...
    try {
      ...
      scriptNode.onaudioprocess = (e) => {
        ...
        if (this.continuousBuffer.length > this.SAMPLE_RATE * 30) {
          const excessSamples =
            this.continuousBuffer.length - this.SAMPLE_RATE * 30;
          this.continuousBuffer = this.continuousBuffer.slice(excessSamples);
          if (this.isRecording) {
            this.recordingStartIndex = Math.max(
              0,
              this.recordingStartIndex - excessSamples
            );
          }
        }
  ...
  async finishRecording() {
    ...
    const actualStartIndex = Math.max(
      0,
      this.recordingStartIndex - this.PRE_RECORD_SAMPLES
    );
    const recordedSamples = this.continuousBuffer.slice(
      actualStartIndex,
      recordingEndIndex
    );
    ...
    this.continuousBuffer = this.continuousBuffer.slice(
      -this.PRE_RECORD_SAMPLES
    );
  }
