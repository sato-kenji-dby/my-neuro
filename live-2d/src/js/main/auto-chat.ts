...
class AutoChatModule {
   ...
   async executeChat() {
       ...
       // 检查其他活动
       if (stateManager.isPlayingTTS || stateManager.isProcessingBarrage || stateManager.isProcessingUserInput) { // 使用 stateManager
       ...
       try {
           ...
           const needScreenshot = await this.screenshotService.shouldTakeScreenshot(prompt); // 使用 this.screenshotService
           if (needScreenshot) {
               try {
