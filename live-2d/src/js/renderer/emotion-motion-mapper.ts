...
class EmotionMotionMapper {
  model: Live2DModel; // 明确 model 属性类型
  ...
  // 解析文本，提取所有情绪标签和位置信息
  parseEmotionTagsWithPosition(text: string): {
    emotion: string;
    startIndex: number;
    endIndex: number;
    fullTag: string;
  ...
  prepareTextForTTS(text: string): {
    ...
    for (let i = emotionTags.length - 1; i >= 0; i--) {
      ...
      purifiedText =
        purifiedText.substring(0, tag.startIndex) +
  ...
  // textLength: 总字符长度
  triggerEmotionByTextPosition(
    position: number,
    textLength: number,
    emotionMarkers: { position: number; emotion: string; motionIndex: number }[]
  ): void {
    // 明确返回类型
    ...
    for (const marker of emotionMarkers) {
      ...
      if (position >= marker.position && position <= marker.position + 2) {
        ...
        console.log(
          `触发情绪: ${marker.emotion}, 动作: ${motionIndex} - ${this.motionDescriptions[motionIndex]}`
        );
  ...
  triggerMotionByEmotion(text: string): string {
    // 明确返回类型
    // 提取第一个情绪标签
    ...
    if (match && match[1]) {
      ...
      if (motionIndex !== undefined) {
        console.log(
          `检测到情绪: ${emotion}, 触发动作: ${motionIndex} - ${this.motionDescriptions[motionIndex]}`
        );
        this.playMotion(motionIndex);
      }
    }
  ...
  playMotion(index: number): void {
    ...
    try {
      ...
      const motionDefinitions =
        ...
        ]; // 更改为 this.model.internalModel.motionManager.definitions
