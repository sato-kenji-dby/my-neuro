...
- [x] 添加待办事项板和专注模式按钮`;
  ...
  onMount(async () => {
    ...
    ipcRenderer.on('play-audio', (event, ...args: unknown[]) => {
      const { audioArrayBuffer, text, cleanedText, wasTranslated } =
        args[0] as {
    ...
    try {
      ...
      // 明确指定 error 类型为 unknown
      ipcRenderer.send('log-to-main', {
        level: 'error',
        message: `加载Live2D模型错误: ${(error as Error).message}`,
      });
    ...
    try {
      ...
      asrProcessor.on('speech-recognized', (...args: unknown[]) => {
        // 接收 unknown[]
      ...
      ipcRenderer.send('log-to-main', {
        level: 'info',
        message: 'ASR 模块已在渲染进程中启动',
      });
    } catch (error: unknown) {
      ipcRenderer.send('log-to-main', {
        level: 'error',
        message: `初始化 ASR 错误: ${(error as Error).message}`,
      });
    }
  ...
  onDestroy(() => {
    if (browser) {
      ...
      window.ipcRenderer.send('log-to-main', {
        level: 'info',
        message: '渲染进程应用已关闭，资源已清理',
      });
      // 通知主进程关闭相关服务
...
<div class="relative h-screen w-screen overflow-hidden">
  ...
  >
    ...
    <div
      id="todo-board"
      class="max-h-80 w-96 rounded-xl border border-slate-700 bg-slate-900/70 p-4 font-sans text-slate-100 shadow-lg backdrop-blur-sm"
    >
      <h2
        class="mb-3 bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-center text-xl font-bold text-transparent"
      >
        待办事项
      </h2>
      <textarea
        bind:value={todoContent}
        class="h-48 w-full resize-none rounded-lg border-2 border-slate-700 bg-slate-800/80 p-3 text-slate-200 transition-all duration-300 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-500"
  ...
  >
    <p
      id="subtitle-text"
      class="font-['Patrick_Hand','ZCOOL_QingKe_HuangYou',sans-serif] text-4xl font-extrabold leading-normal text-white md:text-5xl"
  ...
  >
    ...
    <div id="chat-input-container" class="flex items-center gap-2">
      <input
        ...
        on:keypress={(e) =>
          e.key === 'Enter' && handleTextMessage(chatInputMessage)}
        class="flex-1 rounded-full border-2 border-slate-700 bg-slate-800/80 px-3 py-2 text-slate-200 transition-all duration-300 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-green-500"
      />
      <button
        id="chat-send-btn"
        on:click={() => handleTextMessage(chatInputMessage)}
        class="transform cursor-pointer rounded-full border-none bg-gradient-to-r from-green-500 to-teal-500 px-4 py-2 font-bold text-white transition-all duration-300 hover:scale-105 hover:from-green-600 hover:to-teal-600"
      >
        发送
      </button>
    </div>
