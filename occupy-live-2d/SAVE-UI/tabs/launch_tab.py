    ...
    def launch_mcp_server(self):
        ...
        try:
            ...
            if not os.path.exists(self.server_dir):
                self.log_message(f"服务器目录不存在: {self.server_dir}", "error", is_mcp=True)
                return False
    ...
    def read_mcp_output(self):
        ...
        try:
            for line in iter(self.mcp_process.stdout.readline, ''):
                if line:
                    self.log_message(line.strip(), "normal", is_mcp=True)
        except Exception as e:
    ...
    def read_mcp_error(self):
        ...
        try:
            for line in iter(self.mcp_process.stderr.readline, ''):
                if line:
                    self.log_message(line.strip(), "error", is_mcp=True)
        except Exception as e:
